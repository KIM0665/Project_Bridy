...
## 김진수(팀장)
처음에는 DB 설계와 요구사항 명세서 작성, 기능 설계로 시작했는데, 이 부분에서 시간이 많이 소요되었습니다. <br>
처음이라 수정이 많이 필요했고, 어떤 식으로 구성해야 할지 막막 했으나 구글링을 통해 다양한 참고 자료를 찾아 많은 시행착오를 겪으며 진행할 수 있었습니다. <br>
새의 통계를 구현할 때 복잡성으로 인해 일정이 예상보다 더 오래 걸렸습니다. 특히 **2중 맵을 사용하여 타임리프로 데이터를 가공하는 작업이 가장 도전적**이었습니다. <br>
2중 맵의 키와 값에 대한 가공이 매우 복잡했고 처음 해보는 작업이라 시간이 많이 소요되었지만, 키:값 구조를 사용하여 데이터를 추출하는데 가독성이 좋았다고 생각합니다. <br>
카테고리 별로 필요한 데이터를 가공해서 뿌렸지만, 메인 페이지에도 데이터를 표시해야 했기 때문에 추가 데이터를 가공하여 뷰에 보여주었습니다.  <br>
이 프로젝트를 통해 가장 중요한 것은 요구사항 명세서와 기능 설계, 그리고 팀원들과의 소통과 협업이 중요하다는 것을 배웠습니다. <br>
처음에는 어떻게 설계하고 명세서를 작성해야 할지 어려움을 겪었지만, 팀원들과 지속적인 의사소통을 통해 문제를 빠르게 해결할 수 있다는 것과 중복성을 최소화하기 위해 어떤 코드를 구현해야 하는지 깊게 고민하게 되었습니다. <br>
향후에는 프로젝트를 더 효율적으로 관리하기 위해 초기 단계에서의 기능 설계와 요구사항 명세서에 더 많은 시간을 할애할 것을 느꼈고, <br>
기술적인 측면에서 더 깊이 있는 학습을 통해 향후 프로젝트에서는 이전 경험을 바탕으로 더 나은 결과를 얻을 수 있다고 생각합니다. <br>
